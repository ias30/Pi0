...
修复了wrist相机帧重复问题：使用FFmpeg MJPEG流 + 帧边界搜索 + 主动清空缓冲区 + 预热 + 缓冲区限制策略
"""
...
import select
import fcntl
import sys # >>> CHANGED: 采纳建议 - 添加 import sys
...
# ================================================================
# (这里省略 CameraCollector 类的代码，与之前的版本相同)
...
# ================================================================
#        >>> DIFF: FFmpegWristCamera 应用了所有建议 <<<
# ================================================================

class FFmpegWristCamera:
    """基于FFmpeg的手腕相机采集器 (MJPEG流 + 帧同步 + 主动清空 + 预热 + 缓冲限制)"""
    
    # >>> CHANGED: 采纳建议 - 增加缓冲区大小限制 (字节)
    # 假设 640x480 MJPEG 帧平均大小约 50KB
    # 限制为 10MB 大约可存 200 帧 (约 6-7 秒 @ 30fps)
    MAX_BUFFER_SIZE = 10 * 1024 * 1024 
    # >>> CHANGED: 采纳建议 - 设置预热所需帧数
    WARMUP_FRAMES = 3 
    ...
    def connect(self) -> bool:
        try:
            # >>> CHANGED: 采纳建议 - 添加 -vsync passthrough
            command = [
                'ffmpeg',
    ...
    def _capture_worker(self):
        print(f"INFO: {self.camera_id}: 采集线程已启动 (MJPEG 解析 + 清空 + 预热 + 缓冲限制)")
        ...
        while not self._stop_flag:
            try:
                # 1. 使用 select 等待数据
                rlist, _, xlist = select.select([fd], [], [fd], self.target_interval * 2) # 超时时间稍长，给足读取时间
                ...
                bytes_consumed_this_round = 0 # >>> CHANGED: 用于记录本轮消耗了多少字节

                while True:
                    # >>> CHANGED: 在 processed_offset 之后查找，避免重复扫描
                    soi_index = self._byte_buffer.find(self._soi_marker, processed_offset)
                    
                    if soi_index == -1:
                        # 缓冲区剩余部分没有 SOI 了
                        # >>> CHANGED: 采纳建议 - 正确清理已扫描部分
                        # 清理掉 processed_offset 之前的所有数据
                    ...
                    if eoi_index == -1:
                        ...
                        # 更新 processed_offset 为 0，因为缓冲区开头已改变
                        processed_offset = 0 
                        break # 等待更多数据
                    ...
                    # 继续循环找下一帧，目的是覆盖 last_valid_frame_bytes 为最新帧
                
                # >>> CHANGED: 采纳建议 - 处理结束后，清理已消耗的字节
    ...
    def get_stats(self) -> Dict[str, Any]:
        return {
            ...
            'target_fps': self.fps,
            'last_frame_time': self.last_frame_time,
            'buffer_size': len(self._byte_buffer) # 添加当前缓冲区大小信息
        }
...
class TripleCameraCollector:
    ...
    def __init__(self, resolution: tuple = (640, 480), fps: int = 30):
        ...
        self.camera_high = CameraCollector("camera_high", device_index=0, 
                                           ...
                                           serial_number="031522071209") # 使用正确的序列号
        
        # 两个手腕相机（使用修复后的 FFmpegWristCamera）
        self.camera_left_wrist = FFmpegWristCamera("camera_left_wrist", device_index=6, # 根据 README
                                                     resolution=resolution, fps=fps)
        self.camera_right_wrist = FFmpegWristCamera("camera_right_wrist", device_index=8,# 根据 README
                                                      resolution=resolution, fps=fps)
...
if __name__ == "__main__":
    ...
    lock = threading.Lock() # 保护 stats 字典
    ...
    try:
        ...
        if not collector.connect_all():
             print("❌ 连接失败，退出测试。")
             sys.exit(1) # >>> CHANGED: 采纳建议 - 使用 sys.exit
